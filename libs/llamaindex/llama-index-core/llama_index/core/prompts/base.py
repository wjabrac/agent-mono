...
class BasePromptTemplate(BaseModel, ABC):  # type: ignore[no-redef]
    ...
    def _map_prompt_func_vars(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        """
        Map function-based variables using all available kwargs.

        Function mappings allow template variables to be generated by
        callables instead of fixed values. Each function receives the
        combined set of keyword arguments provided at both prompt creation
        time (``self.kwargs``) and at runtime. The function's return value is
        used for the corresponding template variable, overriding any fixed
        value supplied for that key.

        Args:
            kwargs: Combined runtime and fixed keyword arguments.

        Returns:
            A new kwargs dictionary with function outputs merged with the
            provided values.
        ...
        for key, func in function_mappings.items():
            new_kwargs[key] = func(**kwargs)

        # then, add the provided variables only if not already set by a function
    ...
    def _map_all_vars(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        # map function
        new_kwargs = self._map_prompt_func_vars(kwargs)
